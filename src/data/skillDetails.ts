...
export const SKILL_DETAILS: Record<number, SkillResource> = {
  ...
  35: {
    timeEstimate: '~2 weeks (12 hrs)',
    whatToLearn: [
      'Understand the property graph model — nodes, relationships, properties — and when it beats relational.',
      'Install Neo4j Desktop or Aura free tier and load a sample dataset (Movies, Northwind).',
      'Learn Cypher fundamentals: MATCH, WHERE, CREATE, MERGE, and pattern matching with variable-length paths.',
      'Practice graph algorithms via the Graph Data Science (GDS) library: PageRank, community detection, similarity.',
      'Model a real domain (org chart, supply chain, social network) and write 10 non-trivial Cypher queries.',
      'Integrate Neo4j with Python/TS apps using the official driver and learn indexing & query performance tuning.',
    ],
    tools: ['Neo4j', 'Cypher', 'Neo4j GDS', 'Neo4j Aura', 'Memgraph', 'NetworkX', 'Apache AGE'],
    concepts: ['Property graphs', 'Cypher patterns', 'Variable-length paths', 'Graph algorithms (PageRank, Louvain)', 'Indexing & constraints'],
    resources: [
      { title: 'Neo4j GraphAcademy (free)', url: 'https://graphacademy.neo4j.com/' },
      { title: 'Cypher Manual', url: 'https://neo4j.com/docs/cypher-manual/current/' },
      { title: 'Graph Data Science library', url: 'https://neo4j.com/docs/graph-data-science/current/' },
    ],
    verify: [
      { text: 'Model a real domain in Neo4j and answer 5 questions that would be painful in SQL.' },
      { text: 'Run PageRank or community detection on a real dataset and explain the results.' },
      { text: 'Wire Neo4j into a Python service with a tuned, indexed query under 50 ms.' },
    ],
    miniProject: 'Movie recommender that uses graph similarity over a Neo4j dataset.',
  },
  36: {
    timeEstimate: '~2 weeks (12 hrs)',
    whatToLearn: [
      'Learn what a knowledge graph is — entities, relations, ontologies — and why agents need structured memory.',
      'Practice entity & relation extraction from text using LLMs (with structured output) or spaCy/REBEL.',
      'Design a small ontology for a domain you care about (people, products, events) and load it into Neo4j.',
      'Read Microsoft GraphRAG to see how communities + summaries beat flat RAG on multi-hop questions.',
      'Combine vector search with graph traversal — find the entity, then walk relationships for context.',
      'Add provenance: every fact in the KG should link back to its source document for explainability.',
    ],
    tools: ['Microsoft GraphRAG', 'LangChain LLMGraphTransformer', 'LlamaIndex KG Index', 'spaCy', 'REBEL', 'Neo4j'],
    concepts: ['Entities & relations', 'Ontology design', 'Triple stores', 'Multi-hop reasoning', 'Provenance / source linking'],
    resources: [
      { title: 'Microsoft GraphRAG paper', url: 'https://arxiv.org/abs/2404.16130' },
      { title: 'LangChain Knowledge Graphs', url: 'https://python.langchain.com/docs/use_cases/graph/' },
      { title: 'LlamaIndex KG guide', url: 'https://docs.llamaindex.ai/en/stable/examples/index_structs/knowledge_graph/KnowledgeGraphDemo/' },
    ],
    verify: [
      { text: 'Build a KG from 50 documents and answer a multi-hop question that flat RAG fails on.' },
      { text: 'Show provenance — clicking a fact reveals the source document & passage.' },
      { text: 'Compare KG-RAG vs vector-RAG on the same eval and quantify the difference.' },
    ],
    miniProject: 'Knowledge graph + RAG over a research paper corpus with multi-hop Q&A.',
  },
  37: {
    timeEstimate: '~1.5 weeks (10 hrs)',
    whatToLearn: [
      'Read Anthropic’s Claude docs end-to-end — the API has unique features beyond OpenAI parity.',
      'Master the Messages API, system prompts, prefill, and stop sequences specifically for Claude.',
      'Learn Computer Use — let Claude take screenshots and click around a virtual desktop to do real tasks.',
      'Use Claude Skills (curated capability packs) and Sub-agents to delegate sub-tasks within one workflow.',
      'Ship Artifacts-style outputs (rich, renderable) and use the Files API for long documents.',
      'Build with the Anthropic SDK + MCP so your tools are reusable across Claude Desktop, Cursor, and your apps.',
    ],
    tools: ['Anthropic SDK', 'Claude Computer Use', 'Claude Sub-agents', 'Claude Skills', 'Claude Code', 'Artifacts'],
    concepts: ['Messages API & prefill', 'Computer Use loop', 'Sub-agent delegation', 'Skills packs', 'Extended thinking', 'MCP integration'],
    resources: [
      { title: 'Anthropic API docs', url: 'https://docs.anthropic.com/' },
      { title: 'Claude Computer Use', url: 'https://docs.anthropic.com/en/docs/build-with-claude/computer-use' },
      { title: 'Anthropic Cookbook', url: 'https://github.com/anthropics/anthropic-cookbook' },
    ],
    verify: [
      { text: 'Build a Computer Use agent that performs a real 5-step task in a browser/desktop.' },
      { text: 'Compose a workflow that delegates parts to a sub-agent and aggregates results.' },
      { text: 'Publish an MCP server consumed by Claude Desktop and a custom app.' },
    ],
    miniProject: 'Computer-Use agent that books a meeting room or fills a recurring web form.',
  },
  38: {
    timeEstimate: '~1.5 weeks (10 hrs)',
    whatToLearn: [
      'Understand why context windows are not memory — agents need structured short and long-term stores.',
      'Compare Mem0, Zep, Letta (formerly MemGPT), and LangMem on architecture and trade-offs.',
      'Learn the memory taxonomy: working, episodic, semantic, procedural — and which store fits each.',
      'Build a chatbot that remembers user preferences across sessions using Mem0 or Zep.',
      'Design memory consolidation — summarize episodic events into semantic facts to keep retrieval cheap.',
      'Add memory eviction & PII handling; long-term memory has real privacy & compliance implications.',
    ],
    tools: ['Mem0', 'Zep', 'Letta (MemGPT)', 'LangMem', 'Cognee', 'Pinecone + Postgres'],
    concepts: ['Working vs long-term memory', 'Episodic vs semantic memory', 'Memory consolidation', 'Recall scoring', 'PII & eviction'],
    resources: [
      { title: 'Mem0 docs', url: 'https://docs.mem0.ai/' },
      { title: 'Zep docs', url: 'https://help.getzep.com/' },
      { title: 'Letta (MemGPT)', url: 'https://docs.letta.com/' },
    ],
    verify: [
      { text: 'Ship a chatbot that recalls a fact you told it 10 sessions ago.' },
      { text: 'Run a memory eval — measure recall accuracy and cost per turn at 1k+ stored facts.' },
      { text: 'Implement consolidation that compresses 100 episodic events into <10 semantic ones.' },
    ],
    miniProject: 'Personal AI assistant with persistent memory across web + mobile sessions.',
  },
  39: {
    timeEstimate: '~2 weeks (12–14 hrs)',
    whatToLearn: [
      'Understand the loop: screenshot → reason → act (click/type/scroll) → observe → repeat.',
      'Run Anthropic’s Claude Computer Use reference implementation and watch it complete a real task.',
      'Try OpenAI Operator / CUA model and compare reliability, speed, and cost vs Claude.',
      'Build a browser agent with Playwright + an LLM (or use Browser Use / Stagehand / Skyvern).',
      'Add safety: confirmation prompts for destructive actions, sandboxed browsers, secret redaction.',
      'Measure success rate on a fixed task suite — this is how teams pick a vendor.',
    ],
    tools: ['Claude Computer Use', 'OpenAI Operator', 'Browser Use', 'Stagehand (Browserbase)', 'Skyvern', 'Playwright', 'Selenium'],
    concepts: ['Screenshot-act loop', 'DOM vs vision grounding', 'Sandboxing & safety', 'Action verification', 'Task success eval'],
    resources: [
      { title: 'Claude Computer Use quickstart', url: 'https://docs.anthropic.com/en/docs/build-with-claude/computer-use' },
      { title: 'Browser Use', url: 'https://github.com/browser-use/browser-use' },
      { title: 'Stagehand (Browserbase)', url: 'https://docs.stagehand.dev/' },
    ],
    verify: [
      { text: 'Automate a real 10-step browser workflow with >80% success across 20 runs.' },
      { text: 'Demo a desktop task (Computer Use) — open an app, do a thing, capture the result.' },
      { text: 'Show a safety layer that blocks destructive actions without explicit human approval.' },
    ],
    miniProject: 'Browser agent that monitors job boards and applies on your behalf with approval gates.',
  },
  40: {
    timeEstimate: '~1.5 weeks (10 hrs)',
    whatToLearn: [
      'Learn what makes time-series workloads different — append-heavy, time-bucketed queries, retention policies.',
      'Set up TimescaleDB on top of Postgres and learn hypertables, continuous aggregates, compression.',
      'Try ClickHouse for OLAP at scale — columnar storage, MergeTree engines, materialized views.',
      'Use DuckDB locally for fast ad-hoc analytics on Parquet/CSV without spinning up infra.',
      'Compare query patterns: rollups, percentile latency, gap-filling, downsampling.',
      'Wire one of these into an LLM observability or feature-store use case to see real fit.',
    ],
    tools: ['TimescaleDB', 'ClickHouse', 'DuckDB', 'InfluxDB', 'Apache Druid', 'Materialize'],
    concepts: ['Hypertables & partitioning', 'Continuous aggregates', 'Columnar storage', 'Materialized views', 'Compression & retention'],
    resources: [
      { title: 'TimescaleDB docs', url: 'https://docs.timescale.com/' },
      { title: 'ClickHouse docs', url: 'https://clickhouse.com/docs' },
      { title: 'DuckDB docs', url: 'https://duckdb.org/docs/' },
    ],
    verify: [
      { text: 'Ingest 10M time-series rows and run percentile queries in <500 ms.' },
      { text: 'Build a continuous aggregate that powers a real dashboard.' },
      { text: 'Use DuckDB to analyze a 1 GB Parquet file on your laptop in seconds.' },
    ],
    miniProject: 'LLM observability backend storing token + latency metrics in TimescaleDB.',
  },
};
